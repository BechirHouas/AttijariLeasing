...
public class addAnRecouvreur extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet addAnRecouvreur</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet addAnRecouvreur at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {            
            out.close();
                ...
                if (isEmailValid) {
                    if (pass.length() > 7) {
                        if (pass.equals(passAgain)) {
                            ...
                            String sql = "INSERT INTO  `attijarileasing`.`recouvreurs` "
                                    + "(`idrecouvreur`  ,`email` ,`password` ) "
                                    + "VALUES (NULL ,  ?, SHA1(  ? )); ";
